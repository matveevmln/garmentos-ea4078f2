  ...
  return (
    <div className="mx-auto max-w-[1400px]">
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1.55fr)_minmax(320px,.8fr)]">
        <section><div className="mb-3 flex items-center justify-between"><div><h2 className="t-section">Активные партии</h2><p className="t-meta mt-1">Сначала ближайшие сроки и отклонения</p></div><Button variant="ghost" size="sm" onClick={() => onNavigate("batches")}>Все партии</Button></div><div className="grid gap-3 lg:grid-cols-2"><div><div className="mb-2 flex items-center gap-2"><span className="h-px flex-1 bg-border" /><span className="eyebrow text-[8px]">Текущая</span></div><BatchCard batch={productionBatches[0]} onOpen={onOpenBatch} /></div><div><div className="mb-2 flex items-center gap-2"><span className="h-px flex-1 bg-border" /><span className="eyebrow text-[8px] text-primary">Альтернатива</span></div><AlternativeBatchCard batch={productionBatches[0]} onOpen={onOpenBatch} /></div>{productionBatches.slice(1).map((batch) => <BatchCard key={batch.id} batch={batch} onOpen={onOpenBatch} />)}</div></section>
